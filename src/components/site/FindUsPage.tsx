...
export function FindUsPage() {
  const { findUs } = useSiteContent();

  return (
    <>
      <header className="page-wrap section-pad pt-20 md:pt-28 pb-10 md:pb-14 text-center border-b border-border bg-white">
        <Reveal>
          <p className="text-eyebrow">{findUs.eyebrow}</p>
          <h1 className="font-display text-5xl md:text-7xl lg:text-8xl mt-4 leading-[0.92]">{findUs.title}</h1>
          <p className="text-caption mt-6 max-w-lg mx-auto">{findUs.description}</p>
        </Reveal>
      </header>
      ...
      <div className="page-wrap section-pad py-12 md:py-16 bg-white">
        <Reveal>
          ...
          <p className="text-center text-sm text-muted-foreground mb-16">
            <a href={findUs.atlistMapUrl} target="_blank" rel="noreferrer" className="link-underline">
              Open full map on Atlist
            </a>
          </p>
        </Reveal>

        {findUs.showStockistList && (
          <div className="max-w-3xl mx-auto space-y-14 md:space-y-20">
            {findUs.countries.map((region, ci) => (
        ...
        <Reveal delay={120}>
          <div className="max-w-2xl mx-auto mt-20 md:mt-28 pt-12 border-t border-border text-center">
            <p className="font-display text-2xl md:text-3xl leading-[1.15] italic">{findUs.wholesaleTitle}</p>
            <p className="text-caption mt-5">
              Please email us at{" "}
              <a href={`mailto:${findUs.wholesaleEmail}`} className="link-underline !text-foreground">
                {findUs.wholesaleEmail}
              </a>
            </p>
            <Link to="/contact" className="inline-block mt-8 btn-outline">
              {findUs.wholesaleCtaLabel}
            </Link>
          </div>
        </Reveal>
      </div>
