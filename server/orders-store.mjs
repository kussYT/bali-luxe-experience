...
export async function createPendingOrder(payload) {
  ...
  const order = {
    ...
    shippingCountryCode: payload.shippingAddress?.country || payload.countryCode || null,
    fulfillmentWarehouse: payload.items[0]?.warehouseId || null,
    customerEmail: payload.customerEmail || null,
    customerName: payload.customerName || payload.shippingAddress?.name || null,
...
export async function holdOrderAfterPaymentBlocked(orderId, payload) {
  ...
  orders[idx] = {
    ...
    customerName: payload.customerName || orders[idx].customerName || null,
    customerPhone: payload.customerPhone || orders[idx].customerPhone || null,
    shippingAddress: payload.shippingAddress || orders[idx].shippingAddress || null,
...
export async function markOrderPaid(orderId, paymentMeta) {
  if (isDatabaseConfigured()) {
    ...
    return db.fulfillOrderPayment({
      ...
      customerName: paymentMeta.customerName,
      customerPhone: paymentMeta.customerPhone,
      shippingAddress: paymentMeta.shippingAddress,
  ...
  orders[idx] = {
    ...
    customerName: paymentMeta.customerName || orders[idx].customerName || null,
    customerPhone: paymentMeta.customerPhone || orders[idx].customerPhone || null,
    shippingAddress: paymentMeta.shippingAddress || orders[idx].shippingAddress || null,
    amountTotal: paymentMeta.amountTotal,
